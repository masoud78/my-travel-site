...
export function GalleryTab({ data }: { data: GalleryItem[] }) {
  ...
  const reset = () => {
    setForm({ title: "", image: "", thumbnail: "", destination: "", tourId: "", order: 0 });
    setEditing(null);
    setOpen(false);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <FormModal open={open} onClose={() => setOpen(false)} title={editing ? "ویرایش تصویر گالری" : "تصویر جدید گالری"} onSubmit={handleSubmit} loading={isPending}>
        <div className="space-y-4">
          <div>
            <Label>عنوان</Label>
